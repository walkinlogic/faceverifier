...
class Employee(BaseEmployee):
    def validate(self):
        ...
        if self.image:
            if self.image.startswith('/private/files/'):
                relative_path = self.image.replace('/private/files/', 'private/files/')
            elif self.image.startswith('/files/'):
                relative_path = self.image.replace('/files/', 'public/files/')

        if relative_path:
            image_path = os.path.join(site_path, relative_path)

            is_valid =  is_passport_photo(image_path)
            logger.error(f"is_valid endpoint hit. {str(is_valid)}")
            if is_valid[0]:
                logger.error(f"is_valid TRUE")
                return True
            else:
                frappe.throw(_( "Your Image is not valid. <br>{0}</b>".format( str(is_valid[1]) ) ))
                logger.error(f"is_valid False")
                return False
